...
## Working on the distribution source

Source imports and workspace names follow upstream (`emulate`, `@emulators/*`).
Keep installation examples on the published `@envoy` names. Release preparation
performs the rename in a temporary tree; do not rename source imports for GPR.
Run `pnpm test:release` and `bash scripts/prepare-release.sh <empty-output-dir>`
when changing packaging. Start upstream contributions from `upstream/main` and
land upstream-sync PRs with merge commits. See `docs/upstream-maintenance.md`.
