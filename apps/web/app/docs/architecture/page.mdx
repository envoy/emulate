...
## Distribution source and packages

Maintained source uses upstream `emulate` and `@emulators/*` workspace names.
The Envoy release pipeline creates a temporary copy, applies `@envoy/emulate`
and `@envoy/emulators-*` names with the independent distribution version, and
verifies the packed packages in an isolated consumer before publication to GitHub
Packages. Installation examples use the published names. Upstream contributions
start from `upstream/main`; distribution syncs preserve upstream merge history.
