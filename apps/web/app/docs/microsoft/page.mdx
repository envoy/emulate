...
Refresh requests require the `client_id` and `client_secret` of the client that received the refresh token. Successful requests rotate the refresh token. Legacy refresh records without a stored client binding remain supported.
